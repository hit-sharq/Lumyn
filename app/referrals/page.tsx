  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessage({
          type: "error",
          text: result.error || "Failed to send referral"
        });
      }
    } catch (error) {
      setMessage({
      ...
      <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm mb-10">
          ...
          <div className="flex gap-3">
            <input type="text" readOnly value={referralLink} className="flex-1 rounded-lg border border-slate-300 px-4 py-2 text-sm bg-slate-50" />
            <button onClick={copyToClipboard} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 flex items-center gap-2">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm mb-10">
          ...
          <form onSubmit={handleSubmit} className="flex gap-3">
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="friend@example.com" required className="flex-1 rounded-lg border border-slate-300 px-4 py-2 text-sm" />
          ...
          {message && <p className={`mt-3 text-sm ${message.type === "success" ? "text-green-600" : "text-red-600"}`}>
              {message.text}
            </p>}
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          ...
          {data.referrals.length === 0 ? <div className="px-6 py-12 text-center">
              ...
              {data.referrals.map(referral => <div key={referral.id} className="px-6 py-4 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-900">
                      {referral.referred ? `${referral.referred.firstName} ${referral.referred.lastName}` : referral.referrer ? `${referral.referrer.firstName} ${referral.referrer.lastName}` : "Unknown User"}
                    </p>
                    <p className="text-xs text-slate-500 mt-1">
                      {new Date(referral.createdAt).toLocaleDateString()} • {referral.status}
                    </p>
                  </div>
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${referral.rewardGiven ? "bg-green-50 text-green-700" : referral.status === "completed" ? "bg-yellow-50 text-yellow-700" : "bg-slate-50 text-slate-700"}`}>
                    {referral.rewardGiven ? `Rewarded: ${referral.rewardType || "N/A"}` : referral.status === "completed" ? "Completed" : "Pending"}
                  </span>
                </div>)}
            </div>}
        </div>
