...
export default function CreatorProfile() {
  ...
  return <main className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      <div className="container mx-auto px-4 py-12">
            ...
            <CardContent className="p-0">
              ...
              <div className="px-8 pb-8">
                <div className="flex flex-col md:flex-row gap-6 -mt-16 mb-8 items-end">
                  {creator.avatarUrl ? <Image src={creator.avatarUrl} alt={creator.displayName} width={120} height={120} className="rounded-lg border-4 border-slate-800 object-cover" /> : <div className="w-32 h-32 rounded-lg border-4 border-slate-800 bg-gradient-to-br from-amber-500 to-purple-500 flex items-center justify-center text-4xl font-bold">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
                      <div>
                        ...
                        <div className="flex gap-2 mt-2">
                          {creator.isVerified && <Badge className="bg-blue-500/20 text-blue-300">✓ Verified</Badge>}
                          <Badge className={`${tierInfo.color} text-black`}>{tierInfo.label}</Badge>
                        </div>
                      </div>
                    </div>
                    
                    {creator.bio && <p className="text-slate-300 mb-4 max-w-2xl">{creator.bio}</p>}
                    ...
                    <div className="flex gap-2">
                      <Button onClick={() => setIsFollowing(!isFollowing)} className={isFollowing ? 'bg-slate-600 hover:bg-slate-700' : 'bg-amber-500 hover:bg-amber-600'}>
                        {isFollowing ? 'Following' : 'Follow'}
                      </Button>
                      
                      {creator.website && <a href={creator.website} target="_blank" rel="noopener noreferrer">
                          <Button variant="outline" className="border-slate-600">
                            Website
                          </Button>
                        </a>}
                      
                      {creator.twitter && <a href={`https://twitter.com/${creator.twitter}`} target="_blank" rel="noopener noreferrer">
                          <Button variant="outline" className="border-slate-600">
                            Twitter
                          </Button>
                        </a>}
        ...
        <div>
          ...
          {creator.products.length === 0 ? <Card className="border-slate-700 bg-slate-800/50">
              <CardContent className="text-center py-12">
                <p className="text-slate-400">This creator hasn&apos;t listed any products yet.</p>
              </CardContent>
            </Card> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {creator.products.map(product => <Link key={product.id} href={`/market/${product.id}`}>
                  <Card className="border-slate-700 bg-slate-800/50 hover:border-slate-600 transition h-full overflow-hidden cursor-pointer group">
                    <div className="relative h-48 bg-slate-700 overflow-hidden">
                      {product.previewImage ? <Image src={product.previewImage} alt={product.title} fill className="object-cover group-hover:scale-105 transition" /> : <div className="w-full h-full flex items-center justify-center text-4xl">📦</div>}
                    </div>
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-lg font-bold text-amber-400">
                          KES {product.price.toLocaleString('en-US', {
                      ...
                      {product.tags.length > 0 && <div className="flex flex-wrap gap-1 mt-3">
                          {product.tags.slice(0, 2).map(tag => <Badge key={tag} variant="outline" className="text-xs border-slate-600">
                              {tag}
                            </Badge>)}
                        </div>}
                    </CardContent>
